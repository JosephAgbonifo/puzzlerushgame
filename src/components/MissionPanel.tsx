...
const MissionPanel: React.FC<MissionPanelProps> = ({ missions, isVisible, onClose }) => {
  if (!isVisible) return null;

  const getMissionIcon = (type: MissionType) => {
    switch (type) {
      case 'hourly_puzzle': return Clock;
      case 'streak_challenge': return Zap;
      case 'rare_puzzle_drop': return Gift;
      case 'daily_quest': return Target;
  ...
  const getMissionColor = (type: MissionType) => {
    switch (type) {
      case 'hourly_puzzle': return 'text-blue-400';
      case 'streak_challenge': return 'text-emerald-400';
      case 'rare_puzzle_drop': return 'text-gold-400';
      case 'daily_quest': return 'text-purple-400';
  ...
  const getRewardIcon = (rewardType: string) => {
    switch (rewardType) {
      ...
      case 'special_puzzle': return Gift;
      default: return Target;
    }
  };
        ...
        <div className="bg-gradient-to-r from-primary-700 to-primary-600 p-6 border-b border-gold-400/30">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-gold-300">Active Missions</h2>
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center space-x-3">
                      <div className={`w-10 h-10 rounded-lg bg-primary-600 flex items-center justify-center ${
                        ...
                        <Icon className={`w-5 h-5 ${mission.isCompleted ? 'text-white' : colorClass}`} />
                      </div>
                      <div>
                        <h3 className={`font-bold ${mission.isCompleted ? 'text-emerald-300' : 'text-white'}`}>
                          {mission.title}
                        </h3>
                      ...
                      {mission.requirements.map((req, index) => (
                        <div key={index} className="flex items-center space-x-2 text-sm">
                          <div className={`w-2 h-2 rounded-full ${
                            req.current >= req.target ? 'bg-emerald-400' : 'bg-gray-500'
                          }`} />
                          <span className="text-gray-300">
                            {req.type.replace('_', ' ').toUpperCase()}: {req.current}/{req.target}
                          </span>
                        </div>
                      ))}
                    </div>
                  ...
                  {mission.expiryTime && (
                    <div className="mt-3 pt-3 border-t border-gray-600">
                      <div className="flex items-center space-x-2 text-xs text-gray-400">
                        ...
                        <span>
                          Expires: {new Date(mission.expiryTime).toLocaleString()}
                        </span>
                      </div>
                    </div>
                  )}
