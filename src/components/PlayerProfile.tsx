...
const PlayerProfile: React.FC<PlayerProfileProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "traits" | "history" | "badges"
  >("overview");
  ...
  const TraitCard: React.FC<{ trait: PlayerTrait }> = ({ trait }) => (
    <div className="bg-gray-900 rounded-lg p-4 border border-gold-400/30">
  ...
  const BadgeCard: React.FC<{ badge: Badge }> = ({ badge }) => (
    <div
      className={`bg-primary-700 rounded-lg p-3 border text-center ${
        badge.rarity === "legendary"
          ? "border-gold-400 bg-gradient-to-br from-gold-900/20 to-gold-800/20"
          : badge.rarity === "epic"
          ? "border-purple-400 bg-gradient-to-br from-purple-900/20 to-purple-800/20"
          : badge.rarity === "rare"
          ? "border-blue-400 bg-gradient-to-br from-blue-900/20 to-blue-800/20"
          : "border-gray-400"
      }`}
    >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75 backdrop-blur-sm p-4">
      <div className="bg-gray-950 rounded-xl shadow-2xl border border-gold-400/30 max-w-4xl w-full max-h-[90vh] overflow-hidden">
        {/* Header */}
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-300">
                  Level {profile.level} Puzzle Master
                </p>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {profile.puzzleHistory.slice(0, 5).map((attempt, index) => (
                    <div
                      key={index}
                      className="bg-primary-700/50 rounded-lg p-3 flex justify-between items-center"
                    >
                      <div>
                        <div className="text-sm text-white">
                          Puzzle #{attempt.puzzleId.slice(-6)}
                        </div>
                        <div className="text-xs text-gray-400">
                          {attempt.wordsFound.length} words • {attempt.accuracy}
                          % accuracy
                        </div>
                      </div>
                      <div className="text-right">
                        ...
                        <div className="text-xs text-gray-400">
                          {Math.floor(attempt.completionTime / 60)}:
                          {(attempt.completionTime % 60)
                            .toString()
                            .padStart(2, "0")}
                        </div>
                      </div>
              ...
              {profile.traits.length > 0 ? (
                ...
                <div className="text-center py-8 text-gray-400">
                  ...
                  <p className="text-sm mt-2">
                    Complete puzzles to unlock special abilities!
                  </p>
                </div>
              )}
            </div>
                  ...
                  <div key={index} className="bg-primary-700/50 rounded-lg p-4">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <div className="font-medium text-white">
                          Puzzle #{attempt.puzzleId.slice(-8)}
                        </div>
                        <div className="text-xs text-gray-400">
                    ...
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-300">
                        {attempt.wordsFound.length} words found
                      </span>
                      <span className="text-gray-300">
                        {Math.floor(attempt.completionTime / 60)}:
                        {(attempt.completionTime % 60)
